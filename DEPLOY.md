...
eine Preview-URL unter `<branch>.kittokit.pages.dev`.

## Reservierte URLs

- **Production:** https://kittokit.pages.dev (Custom Domain: https://kittokit.com)
- **Preview-Pattern:** https://&lt;branch-name&gt;.kittokit.pages.dev (wird pro PR automatisch gemintet)
- **CF-Projekt-Name:** `kittokit` (hardcoded in `.github/workflows/deploy.yml` — URL-Wechsel = eine Zeile)
...
Das Projekt `kittokit` wurde in Session 1 manuell reserviert. Einmalig prüfen dass die folgenden Einstellungen im CF-Dashboard stimmen (**Workers & Pages → kittokit → Settings**):
...
```bash
# Triggert CI → production deploy auf kittokit.com
...
CI baut + deployt automatisch auf `feat-mein-neues-tool.kittokit.pages.dev`. Bei Merge nach `main` → production.
...
npx wrangler pages deploy dist --project-name=kittokit
```
...
- **Custom Domain:** kittokit.com ist bereits als Custom Domain in CF Pages konfiguriert und zeigt auf `kittokit.pages.dev`.
