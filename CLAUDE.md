...
- Tokens-only: kein Hex, keine arbitrary-px-Werte in **Components** (`src/components/**/*.svelte`, `src/components/**/*.astro`, `src/layouts/**/*.astro`) und **Token-Files** (`src/styles/tokens.css`, `src/styles/tokens-*.css`). Konkrete Numerik-Vorschläge aus `minimalist-ui` (z.B. `#EAEAEA`, `8px`) werden auf unsere `--color-border` / `--r-md`-Tokens gemappt, NICHT als Hex/px übernommen. **Ausnahme Data-Layer:** in `src/lib/tools/*-presets.ts` und `src/lib/tools/*.ts` sind Hex-Werte erlaubt, wenn sie **Tool-Data** sind (z.B. `hex-rgb-konverter` hat Hex als Input/Output, nicht als visuelle Farbe). Visuelle Farben gehen auch im Data-Layer durch Tokens. Audit 2026-04-21 M-3-01: `ColorConverter.svelte:34-40` hatte Hex-Konstanten in Component — dort verboten, aber in `src/lib/tools/hex-rgb-konverter-presets.ts` ok.
